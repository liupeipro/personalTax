...
public class CalculatorFilterActivity extends BaseActivity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        // data 初始化
        mAmountTypes = TaxApplication.getDaoInstant().getCostItemAmountTypeDao().loadAll();
        
        for (CostItemAmountType item : mAmountTypes) {
            if (item.getName().equals("收入")) {
                mGridInData.clear();
                mGridInData = TaxApplication.getDaoInstant()
                                            .getCostItemTypeDao()
                                            .queryBuilder()
                ...
                mGridOutData.clear();
                mGridOutData = TaxApplication.getDaoInstant()
                                             .getCostItemTypeDao()
                                             .queryBuilder()
                                             .where(CostItemTypeDao.Properties.AmountTypeId.eq(
                                                   item.get_id()))
        ...
        mAccountListData = TaxApplication.getDaoInstant().getCostItemAccountDao().loadAll();
        ...
        if (intentItemId != -1) {
            ...
            mCostItem = TaxApplication.getDaoInstant()
                                      .getCostItemDao()
                                      .queryBuilder()
    ...
    private void onSubmit(CostItem item) {
        ...
        long temp = TaxApplication.getDaoInstant().getCostItemDao().insertOrReplace(mCostItem);
